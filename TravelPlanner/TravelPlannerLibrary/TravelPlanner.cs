using System;
using TravelPlannerLibrary.Modles;
using System.Linq;
...
namespace TravelPlannerLibrary
{
    public class TravelPlanner
    {
        ...
        public DepTime FindConnection(string destination, string start)
        {
            if (destination == null || start == null) return null;
            var schedule = schedules.FirstOrDefault(s => s.City.Equals(destination));
            var depTime = schedule.ToLinz.FirstOrDefault(f => f.Leave.CompareTo(start) >= 0);
            return depTime;
        }
    }
}
